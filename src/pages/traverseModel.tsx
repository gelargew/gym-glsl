...
function Obj() {
    ...
    const head = LowPolyHead()
    console.log(head, 'head')
    const targetPosition = new Float32Array(head.count * 3)
    for (let i = 0; i < head.count; i++) {
        _sample.fromBufferAttribute(head, i * 3)
        targetPosition[i] = _sample.x
        targetPosition[i + 1] = _sample.y
        targetPosition[i + 2] = _sample.z
    }
    geometry.setAttribute('position', new THREE.Float32BufferAttribute(positionArray, 3))
    ...
    return (
        <>
            <points scale={0.2} ref={obj} args={[geometry]} >
